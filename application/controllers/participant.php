...
class Participant extends CI_Controller {
    ...
    public function index() {
        ...
        if (!$data) {
            ...
            if ($this->player_model->check_player_team($user_id) < 11) {
                $temp_data['user_id'] = $user_id;
                $temp_players = $this->player_model->get_all_player();
                $players = array();
                foreach ($temp_players as $player) {
                    $players[$player->position][] = $player;
                }
                $temp_data['players'] = $players;
                $this->load->view('user_pick_team', $temp_data);
            } else {
                $team = $this->player_model->get_team($user_id);
                $team->players = $this->player_model->get_team_player($team->team_id);
                $temp_data['team'] = $team;
                $temp_data['played_team'] = $this->player_model->get_all_played_player();                
                $this->load->view('user_dream_team', $temp_data);
            }
...
//        }
    ...
    public function submit_team() {
        ...
        $this->form_validation->set_rules('team_name', 'Nama Team', 'trim|required');
    ...
    private function sent_email_to_user($tiket, $team_name) {
        ...
        */
        ...
        $this->email->bcc('nindy.panasonic@gmail.com','abhas.panasonic@gmail.com');
